...
namespace CoreTextTypeLayout
{
    CFAttributedStringRef createCFAttributedString (const AttributedString& text)
    {
        ...
        {
            case AttributedString::left:        ctTextAlignment = kCTLeftTextAlignment; break;
            case AttributedString::right:       ctTextAlignment = kCTRightTextAlignment; break;
            case AttributedString::center:      ctTextAlignment = kCTCenterTextAlignment; break;
            case AttributedString::justified:   ctTextAlignment = kCTJustifiedTextAlignment; break;
            default: break;
        }
        ...
        switch (text.getWordWrap())
        {
            ...
            case AttributedString::byWord:      ctLineBreakMode = kCTLineBreakByWordWrapping; break;
    ...
    void drawToCGContext (const AttributedString& text, const Rectangle<int>& area,
                          const CGContextRef& context, const float flipHeight, float* textHeight)
    {
        ...
        if (textHeight != nullptr)
        {
            ...
            *textHeight =  area.getHeight() - lastLineOrigin.y + descent;
        }
    ...
    void createLayout (GlyphLayout& glyphLayout, const AttributedString& text)
    {
        ...
        CGMutablePathRef path = CGPathCreateMutable();
        const CGRect bounds = convertToCGRect (glyphLayout.area);
        CGPathAddRect (path, nullptr, bounds);
        ...
        for (CFIndex i = 0; i < numLines; ++i)
        {
            ...
            Point<float> lineOrigin ((float) cgpLineOrigin.x, glyphLayout.area.getHeight() - (float) cgpLineOrigin.y);

            CGFloat ascent, descent, leading;
            CTLineGetTypographicBounds (line, &ascent,  &descent, &leading);

            GlyphLayout::Line* const glyphLine = new GlyphLayout::Line ((int) numRuns, lineStringRange, lineOrigin,
            ...
            for (CFIndex j = 0; j < numRuns; ++j)
            {
                ...
                const CFIndex runStringEnd = runStringRange.location + runStringRange.length - 1;

                GlyphLayout::Run* const glyphRun = new GlyphLayout::Run ((int) numGlyphs,
                                                                         Range<int> ((int) runStringRange.location,
                                                                                     (int) runStringEnd));
                ...
                const Point<float> linePos (glyphLayout.area.getPosition() + glyphLine->getLineOrigin());

                for (CFIndex k = 0; k < numGlyphs; ++k)
                    glyphRun->addGlyph (new GlyphLayout::Glyph (glyphsPtr[k], linePos.translated (posPtr[k].x,
                                                                                                  posPtr[k].y)));
            }
        }
