...
#define __JUCE_FRAMELABEL_JUCEHEADER__
...
/**
    ...
    editor when clicked.
*/
class JUCE_API  FrameLabel  : public Component,
...
public:
    //==============================================================================
    /** Creates a FrameLabel.

        @param componentName    the name to give the component
        @param labelText        the text to show in the label
    */
    FrameLabel (const String& componentName = String::empty,
    ...
    /** Changes the label text.
        ...
        text, then the class will broadcast a change message to any FrameLabel::Listener objects
        that are registered.
    */
    void setText (const String& newText, bool broadcastChangeMessage);

    void setParagraphs (ScopedPointer<OwnedArray<AttributedString> > newText, bool broadcastChangeMessage);
    ...
    */
    ...
    OwnedArray<AttributedString>& getParagraphs (bool returnActiveEditorContents = false) const;
    ...
    */
    ...
    //==============================================================================
    /**
        A class for receiving events from a FrameLabel.

        You can register a FrameLabel::Listener with a FrameLabel using the FrameLabel::addListener()
        method, and it will be called when the text of the label changes, either because
        of a call to FrameLabel::setText() or by the user editing the text (if the label is
        editable).

        @see FrameLabel::addListener, FrameLabel::removeListener
    */
    ...
    public:
        ...
        /** Called when a FrameLabel's text has changed.
        */
        virtual void labelTextChanged (FrameLabel* labelThatHasChanged) = 0;
    };
...
private:
    ...
    ScopedPointer<OwnedArray<AttributedString> > textValues;
    Font font;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FrameLabel);
};

/** This typedef is just for compatibility with old code - newer code should use the FrameLabel::Listener class directly. */
typedef FrameLabel::Listener FrameLabelListener;

#if JUCE_VC6
 #undef Listener
#endif

#endif   // __JUCE_FRAMELABEL_JUCEHEADER__
